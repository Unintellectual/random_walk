...
#define SCALE 2
#define MAX_TRAIL_LENGTH 5000 // Limit trail length to prevent memory issues
...
int main(int argc, const char *argv[])
{
  ...
  Vector2 trails[num_agents][MAX_TRAIL_LENGTH];
      ...
      for(int i = 0; i < num_agents; i++)
        {
          ...
          if(trail_lengths[i] < MAX_TRAIL_LENGTH)
            {
              trails[i][trail_lengths[i]]
                  = (Vector2){agents[i].x, agents[i].y};
              trail_lengths[i]++;
            }
